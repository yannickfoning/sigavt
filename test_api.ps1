...
if ($myError) {
    Write-Host "  ✗ FAILED: $($myError.Message)" -ForegroundColor Red
...
Write-Host "Test: Login (invalid credentials)" -ForegroundColor Yellow
Write-Host "  POST $baseUrl/api/auth/login" -ForegroundColor Gray
$loginBody = @{email = "test@example.com"; motDePasse = "wrongpassword"} | ConvertTo-Json
...
if ($myError) {
    Write-Host "  ✓ SUCCESS (expected failure)" -ForegroundColor Green
    $results += @{Test = "Login (invalid credentials)"; Result = "SUCCESS"}
}
else {
    Write-Host "  ✗ FAILED (should have failed)" -ForegroundColor Red
    $results += @{Test = "Login (invalid credentials)"; Result = "FAILED"}
}
$myError = $null
Write-Host ""
...
if ($token -ne $null) {
    ...
    # Dashboard Stats
    Write-Host "Test: Dashboard Stats" -ForegroundColor Yellow
    $response = Invoke-RestMethod -Method GET -Uri "$baseUrl/api/dashboard/stats" -Headers $authHeaders -ContentType "application/json" -ErrorVariable myError
    if ($myError) {
        Write-Host "  ✗ FAILED: $($myError.Message)" -ForegroundColor Red
        $results += @{Test = "Dashboard Stats"; Result = "FAILED"}
    }
    else {
        Write-Host "  ✓ SUCCESS" -ForegroundColor Green
        $results += @{Test = "Dashboard Stats"; Result = "SUCCESS"}
    }
    $myError = $null
    Write-Host ""
    
    # Departs
    Write-Host "Test: Departs du jour" -ForegroundColor Yellow
    $response = Invoke-RestMethod -Method GET -Uri "$baseUrl/api/dashboard/departs" -Headers $authHeaders -ContentType "application/json" -ErrorVariable myError
    if ($myError) {
        Write-Host "  ✗ FAILED: $($myError.Message)" -ForegroundColor Red
        $results += @{Test = "Departs du jour"; Result = "FAILED"}
    }
    else {
        Write-Host "  ✓ SUCCESS" -ForegroundColor Green
        $results += @{Test = "Departs du jour"; Result = "SUCCESS"}
    }
    $myError = $null
    Write-Host ""
    ...
    Write-Host "Test: Tracking colis public" -ForegroundColor Yellow
    $response = Invoke-RestMethod -Method GET -Uri "$baseUrl/api/colis/tracking/TEST001" -ContentType "application/json" -ErrorVariable myError
    if ($myError) {
        Write-Host "  ✗ FAILED: $($myError.Message)" -ForegroundColor Red
        $results += @{Test = "Tracking colis public"; Result = "FAILED"}
    }
    else {
        Write-Host "  ✓ SUCCESS" -ForegroundColor Green
        $results += @{Test = "Tracking colis public"; Result = "SUCCESS"}
    }
    $myError = $null
    Write-Host ""
}
